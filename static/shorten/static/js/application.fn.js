 ...
 // Smooth Scroll
  ...
  $.fn.extend({
    modal: function(settings) {
      ...
      var s = $.extend(defaults, settings);
      if(!s.content){
          s.content="请注意，此操作是永久的，单击“继续”后 <strong>不能撤消</strong> this. Click anywhere outside this modal or click <a href='#close' class='close-modal'>关闭</a> to close this.";
      }
      if(!s.title){
      ...
      if(s.confimation){
          ...
          var proceed="<a href='" + s.link + "' class='matbtn btn btn-success pull-right' style='margin-left: 15px;'>Proceed</a> <a href='#' class='matbtn btn btn-danger pull-right close-modal'>取消</a>";
      }
